...
Primer transformem les dades en *pseudocounts* utilitzant la fórmula $y = log_{2}(K+1)$. Això ens permetrà visualitzar la dier resultar útils.
...
Abans, però, filtrem el *DESeqDataSet* per eliminar les files amb un número total de *counts* de 0 o 1, que no ens aportarien cap informació però aportarien soroll innecessari.
...
```{r}
selectedELIvsNIT <- (abs(dfELIvsNIT$log2FoldChange) > 1)
selectedSFIvsNIT <- (abs(dfSFIvsNIT$log2FoldChange) > 1)
selectedELIvsSFI <- (abs(dfELIvsSFI$log2FoldChange) > 1)

selected <- cbind(selectedELIvsNIT, selectedSFIvsNIT, selectedELIvsSFI)
...
Observem que els dos contrastos que comparen teixit infiltrat amb no infiltrat (ELI vs NIT i SFI vs NIT) comparteixen la majoria de gens diferencialment expressats.
